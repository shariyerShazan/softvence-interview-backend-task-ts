...
export const requestForVendor = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: "Failed to send request", success: false, error: error.message });
  }
};

export const acceptVendorRequest = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    if (user.vendorRequest !== "pending") {
      return res.status(400).json({ message: "No pending request found", success: false });
    }
    ...
    return res.status(500).json({ message: "Failed to approve request", success: false, error: error.message });
  }
};
